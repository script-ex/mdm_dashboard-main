...
import { setIdentifyOrganizationalCulturalBarrier } from '../Store/reducers/report';
...
function IdentifyOrganizationalCulturalBarriers() {
  ...
  const state = mdmStoreSelector(
    (state: MDMRootState) =>
      state.report.report.identifyOrganizationalCulturalBarrier
  );
  ...
  const handleRowChange = (tableIndex, rowIndex, value) => {
    ...
    if (tableIndex === 1) {
      ...
      updatedValues[rowIndex].counteract = value;
    }
  ...
  const gotoNext = () => {
    ...
    navigate(getMDMBaseUrl() + '/plan-your-action');
  };

  const isDisabled =
    !values.length ||
    !values
      .map((e) => e.factor.length > 0 || e.counteract.length > 0)
      .reduce((a, b) => a || b);

  return (
    <div className="container">
      <h1> {t('Identify Organizational Cultural Barriers')}</h1>
      <br></br>
      <div>
        <p>
          {t(
            'Organizational cultural barriers are those factors embedded in the way'
          )}
        </p>
        <p>
      ...
      <div className="row">
        <div className="col">
          <p>
            {t(
              'What organizational cultural factors may impede your proposed actions?'
            )}{' '}
        ...
        <div className="col">
          <p>
            {t('How might you counteract on them?')}
            <br></br>
            <a href="mailto:test@test.com">{t('Ask the Coach')}</a>
          </p>
        </div>
...
export default IdentifyOrganizationalCulturalBarriers;
